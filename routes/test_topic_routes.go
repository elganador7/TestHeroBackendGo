...
func setupTestTopicDataRoutes(router *gin.Engine, db *gorm.DB) {
	...
	testTopicApi.Use(auth.JWTAuthMiddleware())
	{
...
func setupTestTopicDataRoutesTest(router *gin.Engine, db *gorm.DB) {
	testTopicCtrl := controllers.TestTopicDataController{
		DB: db,
	}
